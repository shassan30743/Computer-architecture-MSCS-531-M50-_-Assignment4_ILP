...
def create_system(cpu_type, num_threads=1, issue_width=1, branch_prediction=None):
    ...
    if num_threads > 1:
        for i in range(num_threads):
            sys.cpu.interrupts[i].pio = sys.membus.mem_side_ports
            sys.cpu.interrupts[i].int_requestor = sys.membus.cpu_side_ports
            sys.cpu.interrupts[i].int_responder = sys.membus.mem_side_ports
    else:
...
def run_simulation(cpu_type, num_threads=1, issue_width=1, branch_prediction=None):
    ...
    # Display key metrics
    instructions = sys.cpu.totalCommittedInstrs()
    cycles = sys.cpu.numCycles
    ipc = float(instructions) / int(cycles) if cycles > 0 else 0
